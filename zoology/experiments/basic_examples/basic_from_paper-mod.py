...
SEEDS = [21, 42, 63, 84, 78, 121, 142, 163, 184, 178] # , 78]  # or [42, 123, 777]

TRAIN_LEN = 256
TEST_LEN  = 256
NUM_KV    = 64

for seed_ in SEEDS: # , 6.5e-4, 7.5e-4, 9.5e-4, 2.5e-3]:
    for d_state in [64]:          # worth sweeping; recurrent models often like width/state :contentReference[oaicite:5]{index=5}
        for d_model in [64]: # paper sweeps width heavily; start small :contentReference[oaicite:6]{index=6}
            for mod in [True]:
                for lr in [2.5e-3, 4.0e-3, 7.0e-3, 1.0e-2]: #  2.5e-4, 4.5e-4, 5.0e-4, 6.5e-4, 7.5e-4, 9.5e-4, 
                    config = TrainConfig(
                        data=DataConfig(
                        ...
                        weight_decay=0.1,      # matches common MQAR setups :contentReference[oaicite:8]{index=8}
                        learning_rate=lr,
                        seed=seed_,
                    )
