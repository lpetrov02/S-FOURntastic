            ...
            for train_size in dataset_size:
                for num_experts, topk in experts_setups:
                    for lbs in lbs_options:
                        for use_state in state_options:
                            suffix = "_AUX" if lbs == "aux_free" else ("_LBL" if lbs == "lbl" else "")
                            st = "s" if use_state else ""
                            batch_size = 32 if n <= 4 else 16
                            config = TrainConfig(
                                learning_rate=lr,
                                max_epochs=30,
                                weight_decay=0.1,
                                ...
                                model=ModelConfig(
                                    ...
                                    sequence_mixer=ModuleConfig(
                                        name="zoology.mixers.s4d_exp.S4DMoEv3",
                                        kwargs={
                                            "dropout": 0.1,
                                            "d_state": 16,
                                    ...
                                    d_model=MODEL_DIM,
                                    block_type="S4DMoEv3Block",
                                    n_layers=n,
                                ),
                                logger=LoggerConfig(
                                    name="tensorboard",
                                    project_name=f"S4D_v3/model_v3{st}_{num_experts}A{topk}_{n}_layers__lr_{lr}__difficulty_{difficulty}{suffix}",
                                )
                            )
