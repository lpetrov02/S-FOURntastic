...
from fantastic_ssm.ops.triton.layer_norm import _layer_norm_fwd
from mamba_ssm.selective_scan_interface import selective_scan_fn

import selective_scan_cuda


class FantasticInnerFn(torch.autograd.Function):
    @staticmethod
    @custom_fwd
    def forward(ctx, xz, out_proj_weight, out_proj_bias, alpha_probs,
                A, B, C, D, delta, delta_softplus=True, checkpoint_lvl=1):
        """
            xz: (batch, dim, seqlen)
            alpha_probs : (B, E, L)
        ...
        B = alpha_probs.transpose(-1, -2) @ B  # (B, L, N)
        C = alpha_probs.transpose(-1, -2) @ C  # (B, L, N)
        D = alpha_probs.transpose(-1, -2) @ D  # (B, L, H)
        delta = alpha_probs.transpose(-1, -2) @ delta  # (B, L, H)
        ...
        ctx.save_for_backward(xz, x_dbl, out_proj_weight, conv1d_out,
                              A, B, C, D, delta, scan_intermediates, out)
...
def fantastic_inner_fn(
    ...
    return FantasticInnerFn.apply(xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
                              out_proj_weight, out_proj_bias,
